...
features <- trimws(gsub("\\W+"," ", features$V2))

#Get train subject list
subject <-  read.table("UCI HAR Dataset/train/subject_train.txt", sep = "", header = FALSE, col.names = "subject")

#Get train data 
x <- read.table("UCI HAR Dataset/train/X_train.txt", sep = "", header = FALSE, col.names = features)
...
#Get test data
x <- read.table("UCI HAR Dataset/test/X_test.txt", sep = "", header = FALSE, col.names = features)
...
names <- grep("\\.mean\\.|\\.mean$|\\.std\\.|\\.std$", names(dat),value = TRUE)
